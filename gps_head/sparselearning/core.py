...
#from ..modules.transformer_encoder import TransformerEncoderLayer


# from graphtrans_head.modules.transformer_encoder import TransformerEncoderLayer
...
class Masking(object):
    ...
    def structure_init_new(self):
        ...
        layer_num_atten = int(self.args.nhead * self.args.atten_density)

        for lys in range(layers_number):
            layer_nonzeros_index = random.sample([i for i in range(lys*self.args.nhead, (lys+1)*self.args.nhead)], layer_num_atten)
            self.nonzeros_index.extend(layer_nonzeros_index)
            self.nonzeros_attn_heads[self.atten_key_index[lys]] = [ v - lys*self.args.nhead for v in layer_nonzeros_index ]

        print('-'*100)
        print('{} layers, {}/{} heads each layer'.format(layers_number, layer_num_atten, self.args.nhead))
        ...
        for name_cur in self.atten_masks.keys():
            ...
            break
        muti_head_dim = int(atten_dim / self.args.nhead) # 128/ 4
        self.atten_mask_shape = (atten_dim0, muti_head_dim)

        for nonzero_idx in self.nonzeros_index:

            layer_index = int(nonzero_idx / self.args.nhead)
            key = self.atten_key_index[layer_index]
            index_inside_layer = nonzero_idx % self.args.nhead
    ...
    def add_module(self, module, sparse_init='ERK', grad_dic=None):
        ...
        for name, tensor in module.named_parameters():
            ...
            # for attention head
            #############################
            if self.args.head_sparse:
                if "attn.in_proj_weight" in name:
                    self.names.append(name)
                    self.atten_masks[name] = torch.zeros_like(tensor, dtype=torch.float32, requires_grad=False).to(self.device)
                    print("Add Sparse Module for Atten Heads. Layer :{} -- Tensor Size:{}!".format(name, tensor.size()))
        ...
        self.init_death_rate(self.growth_death_ratio)

        print("init Attention Mask Done!")
        print("="*40)
    ...
    def apply_mask(self):
        ...
        for module in self.modules:
            for name, tensor in module.named_parameters():
                if name in self.atten_masks:
                    tensor.data = tensor.data*self.atten_masks[name]
                    #print("name:{}; attn_masks:{}".format(name, self.atten_masks[name]))
                    #print("Trying to Apply Mask on {}".format(name))
                    if 'momentum_buffer' in self.optimizer.state[tensor]:
                        self.optimizer.state[tensor]['momentum_buffer'] = self.optimizer.state[tensor]['momentum_buffer']*self.atten_masks[name]
    ...
    def truncate_weights(self, step=None):
        ...
        if curr_prune_iter >= ini_iter and curr_prune_iter <= final_iter - 1:
            ...
            atten_dim = muti_head_dim * self.args.nhead
            # layer-wise death and growth
            layers_number = len(list(self.atten_masks.keys()))
            module_name_list = list(map(str.__add__, ["convs." for i in range(layers_number)], [str(i) for i in range(layers_number)]))
            prYellow(module_name_list)
            for module in self.modules:
                for name, m in module.named_modules():
                    #print("Name:{}; Module:{}".format(name, m))
                    #if isinstance(m, TransformerEncoderLayer):
                    #if name == "transformer_encoder.transformer.layers.1":
                    ...
                    if name in module_name_list:
                        ...
                        disabled_index = [i for i in range(self.args.nhead) if i not in enabled_index]
