...
def gene_arg():
    ...
    group.add_argument('--head_sparse', action='store_true', help='If Sparse Adj.')
    ...
    group.add_argument('--decay-schedule', type=str, default='cosine', help='The decay schedule for the pruning rate. Default: cosine. Choose from: cosine, linear.')
    group.add_argument('--growth_schedule', type=str, default='gradient', help='The growth schedule. Default: gradient. Choose from: gradient, momentum, random.')
    group.add_argument('--atten_density', type=float, default=0.75, help='The ratio of remained heads')
    group.add_argument('--growth_death_ratio', type=float, default=0.5, help='The ratio of death and regrowth')
...
class GPS(torch.nn.Module):
    def __init__(self, fea_dim, channels: int, num_layers: int, num_tasks):
        super().__init__()
...
args.device = torch.device('cuda:{}'.format(args.devices) if torch.cuda.is_available() else 'cpu')
model = GPS(fea_dim=num_features, channels=64, num_layers=args.num_layers, num_tasks = num_tasks).to(device)
...
if args.sparse:
    ...
    mask = Masking(optimizer, prune_rate=args.prune_rate, growth_death_ratio = args.growth_death_ratio, death_mode=args.prune, prune_rate_decay=decay, growth_mode=args.growth,
