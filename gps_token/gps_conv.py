...
def scatter(logits, index, k):
   ...
   #print('bs = {}'.format(bs))
    ...
    output = torch.zeros_like(logits)#.cuda() # CUDA !!!!!
...
def gumbel_softmax(logits, k, tau=1, hard=False, eps=1e-10, dim=-1):
    ...
    def _gen_gumbels():
        gumbels = -torch.empty_like(logits).exponential_().log() #.cuda() # # CUDA !!!!!
...
class GPSConv(torch.nn.Module):
    ...
    def __init__(
        ...
        self.gumbel = torch.nn.Linear(channels, 1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        h, mask = to_dense_batch(x, batch) # h size: [batch size, Length, feature_dim]

        B, L, C = h.shape

        tau = 1
        number = int(L*self.token_ratio)

        if tau > 0:
            token_scores = self.gumbel(h)
            ...
            token_mask[:,-1] = 1.
            #print( "NUmber of Zeros: {}".format(torch.count_nonzero(token_mask==0)))
