...
def add_sparse_args(parser):
    ...
    parser.add_argument('--prune_rate', type=float, default=0.50, help='The pruning rate / death rate for Zero-Cost Neuroregeneration.')
...
class Masking(object):
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    if name == "edge_weight_train":
                        ...
                        num_params_to_keep = int(len(score) * (1 - curr_prune_rate))
                        #print("Appling Sparse Mask --- Model Weight: {} !".format(name))
    ...
    def add_module(self, module, sparse_init='ERK', grad_dic=None):
        ...
        if self.args.rm_first:
            for name, tensor in module.named_parameters():
                if 'node_encoder.weight' in name or 'pred_linear.weight' in name:
                    self.masks.pop(name)
