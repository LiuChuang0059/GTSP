...
def gene_arg():
    ...
    group.add_argument('--num_encoder_layers', type=int, default=5,
                        help='number of GNN message passing layers (default: 5)')
...
model = GPS(fea_dim=num_features, channels=64, num_layers=10, num_tasks = num_tasks).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-5)
model_total_para = count_parameters(model)
...
results_to_file(args, best_test_acc, best_val_acc, model_total_para)
